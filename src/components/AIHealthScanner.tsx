...
import { Camera, Scan, CheckCircle, AlertTriangle, RefreshCw, Eye, Hand, Users } from 'lucide-react';
    ...
    setTimeout(() => {
      const mockResults = {
        general: {
          overallScore: 85,
          findings: [
            { type: 'positive', message: '전반적인 안색이 건강해 보입니다', confidence: 90 },
            { type: 'warning', message: '약간의 피로 징후가 보입니다', confidence: 75 },
            { type: 'neutral', message: '수분 섭취를 늘려보세요', confidence: 80 }
          ],
          recommendations: [
            '충분한 수면을 취하세요 (7-8시간)',
            '하루 2L 이상의 물을 마시세요',
        ...
        eyes: {
          overallScore: 78,
          findings: [
            { type: 'warning', message: '눈의 피로도가 높아 보입니다', confidence: 85 },
            { type: 'positive', message: '눈 주변 혈색은 양호합니다', confidence: 80 }
          ],
          recommendations: [
            '20-20-20 규칙을 실천하세요',
            '화면 사용 시간을 줄여보세요',
        ...
        skin: {
          overallScore: 82,
          findings: [
            { type: 'positive', message: '피부 톤이 균등하고 건강해 보입니다', confidence: 88 },
            { type: 'neutral', message: '약간의 건조함이 관찰됩니다', confidence: 70 }
          ],
          recommendations: [
            '보습제를 규칙적으로 사용하세요',
            '자외선 차단제를 꼭 발라주세요',
      ...
      setScanResult(mockResults[selectedScanType as keyof typeof mockResults]);
      setIsScanning(false);
      stopCamera();
    }, 3000);
  };
          ...
          <div className="space-y-6">
            ...
            <Card className="card-elevated border-glow">
              <CardContent className="p-6 text-center">
                <div className="w-24 h-24 mx-auto mb-4 relative">
                  <svg className="w-24 h-24 transform -rotate-90">
                    ...
                    <circle
                      ...
                      strokeDashoffset={`${2 * Math.PI * 40 * (1 - scanResult.overallScore / 100)}`}
                ...
                <Badge 
                  variant={scanResult.overallScore >= 80 ? 'default' : scanResult.overallScore >= 60 ? 'secondary' : 'destructive'}
                  className="text-sm"
                >
                  {scanResult.overallScore >= 80 ? '우수' : scanResult.overallScore >= 60 ? '보통' : '개선 필요'}
                </Badge>
              </CardContent>
            </Card>
                ...
                {scanResult.findings.map((finding: any, index: number) => (
                  <div key={index} className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                    ...
                    <div className="flex-1">
                      ...
                      <Progress value={finding.confidence} className="mt-2 h-1" />
                      <p className="text-xs text-muted-foreground mt-1">신뢰도: {finding.confidence}%</p>
                    </div>
                  </div>
                ))}
              </CardContent>
