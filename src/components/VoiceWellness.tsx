...
import { Mic, MicOff, Play, Pause, BarChart3, Brain, Heart, Zap } from 'lucide-react';
  ...
  const analyzeVoice = async () => {
    ...
    setTimeout(() => {
      const mockResult = {
        ...
        emotionalState: ['calm', 'excited', 'neutral', 'tired'][Math.floor(Math.random() * 4)],
        ...
        insights: [
          { category: '스트레스', value: Math.floor(Math.random() * 40) + 30, color: 'text-red-500' },
          { category: '피로도', value: Math.floor(Math.random() * 50) + 25, color: 'text-orange-500' },
          { category: '활력도', value: Math.floor(Math.random() * 40) + 50, color: 'text-green-500' },
          { category: '음성 품질', value: Math.floor(Math.random() * 30) + 70, color: 'text-blue-500' }
        ]
      };
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card className="card-elevated">
                <CardContent className="p4 text-center">
                  ...
                  <div className="text-2xl font-bold">{100 - analysisResult.stressLevel}</div>
                  <div className="text-sm text-muted-foreground">웰니스 지수</div>
                </CardContent>
              </Card>
              ...
              <Card className="card-elevated">
                <CardContent className="p-4 text-center">
                  ...
                  <div className="text-2xl font-bold">{analysisResult.energyLevel}</div>
                  <div className="text-sm text-muted-foreground">에너지 레벨</div>
                </CardContent>
              </Card>
            </div>
            ...
            <Card className="card-elevated">
              ...
              <CardContent>
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-4 h-4 rounded-full ${getEmotionalStateColor(analysisResult.emotionalState)}`}></div>
                  <span className="font-medium">{getEmotionalStateText(analysisResult.emotionalState)}</span>
                  <Badge variant="secondary">{analysisResult.voiceHealth}% 음성 품질</Badge>
                </div>
                  ...
                  {analysisResult.insights.map((insight: any, index: number) => (
                    <div key={index} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2 flex-1 ml-4">
                        <Progress value={insight.value} className="flex-1 h-2" />
